...
float3 AOMultiBounce( float3 BaseColor, float AO )
{
	...
	return max( AO, ( ( AO * a + b ) * AO + c ) * AO ); // == max(AO, a*AO^3 + b*AO^2 + c*AO) >= AO
}
	...
	inout FPixelShaderOut Out)
{
	...
	GBuffer.SpecularColor = lerp(0.08f * Specular, BaseColor, Metallic);
	...
	GBuffer.GBufferAO = AOMultiBounce( Luminance( GBuffer.SpecularColor ), MaterialAO ).g; // > MaterialAO
	...
	GetPrecomputedIndirectLightingAndSkyLight(MaterialParameters, Interpolants, BasePassInterpolants, LightmapVTPageTableResult, 
		GBuffer, DiffuseDir, VolumetricLightmapBrickTextureUVs, 
		DiffuseIndirectLighting, SubsurfaceIndirectLighting, IndirectIrradiance);
	Color += (DiffuseIndirectLighting * DiffuseColorForIndirect) * AOMultiBounce( GBuffer.BaseColor, MaterialAO );
#endif
	...
	EncodeGBuffer(GBuffer, Out.MRT[1], Out.MRT[2], Out.MRT[3], Out.MRT[4], OutGBufferE, OutVelocity, QuantizationBias);
	...
	out float OutIndirectIrradiance)
{
	...
	OutDiffuseLighting += SkyDiffuseLighting;// Sky lighting must contribute to IndirectIrradiance for ReflectionEnvironment lightmap mixing

#if HQ_TEXTURE_LIGHTMAP ||  PRECOMPUTED_IRRADIANCE_VOLUME_LIGHTING
	OutIndirectIrradiance = Luminance(OutDiffuseLighting);
#endif
}

void GetSkyLighting(FMaterialPixelParameters MaterialParameters, VTPageTableResult LightmapVTPageTableResult, 
	FGBufferData GBuffer, float3 WorldNormal, float2 LightmapUV, uint LightmapDataIndex, float3 SkyOcclusionUV3D, 
	out float3 OutDiffuseLighting, out float3 OutSubsurfaceLighting)
{
	OutDiffuseLighting = 0;
	OutSubsurfaceLighting = 0;

#if ENABLE_SKY_LIGHT  // when stationary sky or translucent material
	float SkyVisibility = 1;
	float GeometryTerm = 1;
	float3 SkyLightingNormal = WorldNormal;
	
	#if HQ_TEXTURE_LIGHTMAP || PRECOMPUTED_IRRADIANCE_VOLUME_LIGHTING
		if (ShouldSkyLightApplyPrecomputedBentNormalShadowing()) // when stationary sky
		{
			float3 NormalizedBentNormal;
			...
			#else
				NormalizedBentNormal = WorldNormal;
			#endif
	...
	#endif
	// Apply bent normal AO to the sky diffuse		
	OutDiffuseLighting += (GetEffectiveSkySHDiffuse(SkyLightingNormal) * ResolvedView.SkyLightColor.rgb) * (SkyVisibility * GeometryTerm);
#endif
}
